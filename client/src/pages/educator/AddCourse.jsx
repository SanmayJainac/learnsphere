        ...
        <div className='flex items-center justify-between flex-wrap'>
          ...
          <div className='flex md:flex-row flex-col items-center gap-3'>
            ...
            <label htmlFor="thumbnailImage" className='flex items-center gap-3'>
              <img src={assets.file_upload_icon} alt="" className='p-3 
              bg-blue-500 rounded' />
        ...
        <div>
            ...
            <div key={chapterIndex} className='bg-white border rounded-lg mb-4'>
              <div className='flex justify-between items-center p-4 border-b'>
                <div className='flex items-center'>
                  ...
                  onClick={()=>handleChapter('remove',chapter.chapterId)} className='cursor-pointer' />
              </div>
              {!chapter.collapsed && (
                <div className='p-4'>
                  {chapter.chapterContent.map((lecture,lectureIndex) => (
                    ...
                    items-center mb-2'>
                      <span>{lectureIndex+1} {lecture.lectureTitle}-
                        {lecture.lectureDuration} mins -<a href={lecture.lectureUrl} target='_blank' 
                        className='text-blue-500'>Link</a> - {lecture.isPreviewFree ?
                          'Free Preview' : 'Paid Preview'} </span>
                          <img src={assets.cross_icon} alt="" 
          ...
          {showPopup && (
            ...
            bg-gray-800 bg-opacity-50'>
              <div className='bg-white text-gray-700 p-4 rounded relative w-fill
              max-w-80 '>
                <h2 className='text-lg font-semibold mb-4'>Add Lecture</h2>
                <div className='mb-2'>
                    <p>Lecture Title</p>
                    <input onChange={e => setLectureDetails({ ...lectureDetails,lectureTitle:e.target.value})}
                    value={lectureDetails.lectureTitle} type="text" className='mt-1 block w-full border rounded
                    py-1 px-2' />
                </div>
                 <div className='mb-2'>
                    <p>Duration (minutes)</p>
                    <input onChange={e => setLectureDetails({ ...lectureDetails,lectureDuration:e.target.value})}
                    value={lectureDetails.lectureDuration} type="number" className='mt-1 block w-full border rounded
                    py-1 px-2' />
                </div>
                 <div className='mb-2'>
                    <p>Lecture URL</p>
                    <input onChange={e => setLectureDetails({ ...lectureDetails,lectureUrl:e.target.value})}
                    value={lectureDetails.lectureUrl} type="text" className='mt-1 block w-full border rounded
                    py-1 px-2' />
                </div>
                 <div className='flex gap-2 my-4'>
                  <p>Is Preview Free?</p>
                  <input onChange={e => setLectureDetails({ ...lectureDetails,isPreviewFree:e.target.checked})}
                  checked={lectureDetails.isPreviewFree} type="checkbox" className='mt-1 scale-125' />
                 </div>
